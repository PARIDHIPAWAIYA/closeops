...
> **Placeholder — filled in after the recorded runtime close.** Do not cite these until the close
> has run.

- Controls after close (`closeops check` on `main`): _TBD — target 10/10 green._
- Funnel: exact _TBD_ → rules-only _TBD_ → agent _TBD_ → after review _TBD_.
- Run 1 → run 2 auto-post rate: _TBD → TBD_.
- Proposal acceptance rate: _TBD_.
- Dodo mode used for the demo: **fixture** _(update if live)_.
- Screenshots: AO board (build + close sessions), Needs You queue, red + green CI comments,
  Neatlogs session, reconciled Dodo payout, `close-report.md` funnel: _TBD_.
