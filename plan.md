...
- **Neatlogs (free tier; verified against neatlogs 1.4.21 on this machine):** at the top of `closeops/cli.py`: `neatlogs.init(api_key=os.environ.get("NEATLOGS_API_KEY"), workflow_name="closeops", tags=["syndicate", period])`, skipped entirely when the env var is unset. Decorate `prepare`, `decide`, `apply` and `check` with `@neatlogs.span(kind="WORKFLOW", name=f"<step>:{task}", tags=[task], session_id=f"close-{period}")`, and each control function with `@neatlogs.span(kind="TOOL", name="C7")`, so a close run shows as one session with per-step spans, candidate counts, decision counts and control outcomes as captured output. Get the trace id inside a span with `carrier = {}; neatlogs.inject_trace_context(carrier)`, then parse the W3C `traceparent` it writes (`00-<32-hex trace id>-<16-hex span id>-<flags>`) for the trace id (verified live: neatlogs 1.4.21 runs an isolated tracer provider, so `opentelemetry.trace.get_current_span()` returns an invalid span with trace id 0 even inside `@neatlogs.span`; `neatlogs.extract_trace_context(carrier)` is for cross-process propagation, not for reading the current id) and record it into `decisions.json` and each exception's `trace:`; fall back to `workflow_name+timestamp` when injection yields nothing. Smoke-tested 12:37 IST: `init` + one `WORKFLOW` span → `POST ingest.neatlogs.com/v1/traces` returned 200.
